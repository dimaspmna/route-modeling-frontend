...
const FleetRoute = () => {
    ...
    const handleCalculate = async () => {
        ...
        if (!isVRP) {
            // Mode non-VRP (existing code)
            results = selectedShips.map((ship) => {
                const pointsWithStart = [
                    ship,
                    ...selectedPoints.filter((p) => p.ship_request_id !== ship.ship_request_id),
                ];
                ...
                const result = normalizeResult(ship, routeData);
                return result;
            });
        } else {
            // ✅ MODE VRP - dengan intermediate points
            const body = {
                ...
                // ✅ Baru: Kirim data intermediate points
                intermediate_points: intermediatePoints.map((point, index) => ({
                    ...
                    is_intermediate: true,  // Flag khusus untuk intermediate
                })),
                ...
                return_to_start: !returnToIntermediate, // Jika tidak kembali ke intermediate, kembali ke start
                                            ...
                                            >
                                                ...
                                                <div className="mt-4 pt-3 border-t border-gray-200">
                                                    ...
                                                    <div className="text-sm space-y-1">
                                                        <div className="font-semibold">1. {ship.name} (Titik Awal)</div>
                                                        {route.optimal_route.slice(1, -1).map((point, index) => {
                                                        ...
                                                        <div className="font-semibold">
                                                            {route.optimal_route.length}. {returnToIntermediate && returnIntermediatePoint
                                                                ? returnIntermediatePoint.name + ' (Titik Akhir)'
                                                                : ship.name + ' (Kembali ke Titik Awal)'}
                                                        </div>
                                                    </div>
