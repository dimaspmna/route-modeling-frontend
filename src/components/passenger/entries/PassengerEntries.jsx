...
import EditPassengerEntries from "./EditPassengerEntries";
...
const PassengerEntries = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const res = await API.get("/get-passenger-request");
                setData(res.data || []);
            } catch (err) {
    ...
    const handleQuickUpdate = async (item, newStatus) => {
        try {
            await API.put(`/passenger-request/${item.ship_request_id}`, {
                ...item,
...
export default PassengerEntries;
